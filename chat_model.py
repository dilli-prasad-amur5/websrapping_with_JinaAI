...
# Main function to handle the user's mode choice
async def main():
    mode = input(colored("Enter 'r' for reading a URL or 's' for searching a query: ", 'cyan')).strip().lower()
    messages = []
    ...
    if mode == 'r':
        ...
        print(colored("Invalid input. Please enter 'r' or 's'.", 'red'))
    
    # Call GPT-4 with the collected messages
    response = await call_gpt4_with_streaming(messages)
    print(response)

# Function to handle adding or replacing content in messages
def update_messages(messages, content):
    if not messages:
        messages = [{"role": "system", "content": f"please answer user questions based on all the contents provided: \n\ninitial content:\n{content}"}]
    else:
        while True:
            action = input(colored("Do you want to add this content to the context or replace it? (a/r): ", 'cyan')).lower()
            if action in ['a', 'r']:
                break
            else:
